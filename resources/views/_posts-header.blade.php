        ...
        <div class="lg:flex lg:inline-flex lg:items-center relative bg-gray-100 rounded-xl">
            <div x-data="{ show: false }" @click.away="show = false">
                ...
                <div x-show="show" class="py-2 absolute bg-gray-100 w-full mt-2 rounded-xl z-50" style="display: none">
                    <a href="/" class="block text-left px-3 text-s leading-6 hover:bg-gray-300 focus:bg-gray-300">
                        All
                    </a>

                    @foreach ($categories as $category)
                        <a href="/categories/{{ $category->slug }}" class="block text-left px-3 text-s leading-6 hover:bg-gray-300 focus:bg-gray-300">
                            {{ $category->name }}
                        </a>
                    @endforeach    
                </div>
            ...
            <div x-data="{ show: false }" @click.away="show = false">
                ...
                <div x-show="show" class="py-2 absolute bg-gray-100 w-full mt-2 rounded-xl z-50" style="display: none">
                    ...
                    @foreach ($authors as $author)
                        <a href="/authors/{{ $author->username }}" class="block text-left px-3 text-s leading-6 hover:bg-gray-300 focus:bg-gray-300">
                            {{ $author->name }}
                        </a>
                    @endforeach    
                </div>
